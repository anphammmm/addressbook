...
bool PeopleBook::loadFromCsv(const std::string& path) {
    ...
    data.clear();
    std::string line;
    // giả sử header: Name,Age,Address,Phone,Title,Salary
    std::getline(in, line); // bỏ header
...
std::vector<std::pair<std::string, dataEmployee>> PeopleBook::getAll() const {
    ...
    for (const auto& p : data) {      // 'data' là map<string, dataEmployee> của bạn
        res.push_back(p);
    }
    return res;
}
